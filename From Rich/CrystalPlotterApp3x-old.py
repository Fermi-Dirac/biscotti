...
class Crystal():
    ...
    def mkCrystalPoints(self,Run,N):
        ...
        sV = np.ones(len(x))*0.5
        return x,y,z,c,sV
    
    def plotCrystal(self,Run,N):
        x,y,z,c,sV = self.mkCrystalPoints(Run,N)
        pts = mlab.quiver3d(x, y, z, sV, sV, sV, scalars=c, mode="sphere", scale_factor=1,vmin=0,colormap='jet')
...
from tkinter import filedialog as fDog
...
class CrystalInteract(HasTraits):
    ...
    @on_trait_change('run_n,size,scene.activated')
    def update_plot(self):
        if self.run_n > self.maxn:
            self.run_n = self.maxn
        x,y,z,c,sV=self.crystal.mkCrystalPoints(self.run_n,self.size)
        self.scene.mlab.clf()
        self.plot = self.scene.mlab.quiver3d(x, y, z, sV, sV, sV, scalars=c, mode="sphere", scale_factor=1,vmin=0,colormap='jet')
